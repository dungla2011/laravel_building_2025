    ...
    private function checkAndRestartServer(): void
    {
        ...
        if (!getenv('CI') && !getenv('GITHUB_ACTIONS')) {
            ...
            if (is_resource($process)) {
                // Close pipes to detach process (this is the key!)
                fclose($pipes[0]);
                fclose($pipes[1]); 
                fclose($pipes[2]);
        ...
        die("\n\n ❌ Failed to start Laravel server after $maxAttempts attempts");
    }
